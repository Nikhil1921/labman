...
class Notifications extends Admin_controller  {
	...
	{
		parent::__construct();
		$this->path = $this->config->item('notifications');
	}

	private $table = 'notifications';
	protected $redirect = 'notifications';
    ...
    {
        check_ajax();
        $this->load->model('Notifications_model', 'data');
    ...
    public function add()
	{
        ...
        if ($this->form_validation->run() == FALSE)
            ...
            if($id) $this->send_notification($post);
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['url'] = $this->redirect;
            $data['data'] = $this->main->get($this->table, 'n_name, image', ['id' => d_id($id)]);
            ...
            if($uid) $this->send_notification($post);
    ...
    {
        $users = $this->main->getAll('users', 'token', ['token != ' => '']);
        foreach($users as $u)
            send_notification(APP_NAME, $post['n_name'], $u['token'], $this->config->item('user-token'), $this->path.$post['image']);
    }
